...
public interface BetOptionCrudRepository extends CrudRepository<BetOption, Integer> {
    ...
    public List<BetOption> findByColor(@Param("color") String color);
    @Query(value = "SELECT bo FROM bet_options bo WHERE bo.option = :option", nativeQuery = true)
    public List<BetOption> findByNumber(@Param("option") String option);
}
